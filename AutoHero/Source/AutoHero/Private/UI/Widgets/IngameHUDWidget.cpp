...
void UIngameHUDWidget::LoadListUnit()
{
	...
	if (PlayerState)
	{
		...
		{
			FUnitData* UnitData = UUnitDataManager::Get()->GetUnitDataByType(UnitDTO.UnitType);
			if (UnitData)
			{
...
void UIngameHUDWidget::NativeOnDragLeave(const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
	...
	if (PlaceholderUnit == nullptr)
	{
		...
		{
			FUnitData* UnitData = UUnitDataManager::Get()->GetUnitDataByType(CurrentSelectedSlot->UnitID);
